...
#include "ht/coroutine/block_wait.hpp"
#include "ht/coroutine/detached_task.hpp"
#include "ht/coroutine/task.hpp"
...
TEST_CASE("do not exec before await", "[coroutine][task]") {
  ...
  auto v = ht::block_wait([&]() -> ht::task<int> {
...
TEST_CASE("lots of tasks", "[coroutine][task]") {
  ...
  ht::block_wait([](auto func) -> ht::task<> {
    int sum = 0;
